// Implementação do histograma e das estatísticas da imagem.
// O arquivo também cuida da renderização do gráfico na interface
// e da exportação dos dados usados no modo headless.
// Integrantes:
...
namespace histogram_constants {
    ...
    // Essas cores mantêm o gráfico legível tanto na interface quanto na exportação em PNG.
    constexpr SDL_Color BACKGROUND{40, 40, 40, 255};
    constexpr SDL_Color BORDER{200, 200, 200, 255};
    constexpr SDL_Color PRIMARY_BARS{100, 150, 200, 255};
    constexpr SDL_Color PLOT_BACKGROUND{20, 20, 20, 255};
    constexpr SDL_Color AXIS_COLOR{200, 200, 200, 255};
    constexpr SDL_Color EXPORT_BARS{90, 170, 255, 255};

    // Os limiares seguem uma classificação simples para facilitar a leitura do resultado.
...
namespace classifications {
    ...
    constexpr std::string_view MEDIUM_INTENSITY = "média";
    constexpr std::string_view BRIGHT_INTENSITY = "clara";

    constexpr std::string_view LOW_CONTRAST = "baixo";
    constexpr std::string_view MEDIUM_CONTRAST = "médio";
...
[[nodiscard]] constexpr Uint32 readPixelValue(const Uint8* pixel_base,
                                              int pixel_index,
...
void renderHistogramBars(SDL_Renderer* renderer,
                         int x,
                         int y,
                         int width,
                         int height,
                         ...
                         int max_value,
                         const SDL_Color& bar_color) noexcept {
...
void setSurfacePixel(SDL_Surface* surface,
                     int x,
                     int y,
...
[[nodiscard]] std::pair<float, float> calculateHistogramStatistics(
    ...
    // A média e o desvio padrão são calculados diretamente sobre a distribuição de intensidades.
...
void Histogram::calculate(SDL_Surface* image_surface) {
    // Cada cálculo parte do zero para evitar misturar dados de imagens anteriores.
    data_.fill(0);
    mean_ = 0.0f;
    std_deviation_ = 0.0f;
    max_value_ = 0;
    ...
    // O acesso direto aos pixels só é seguro enquanto a superfície estiver travada.
    ...
    // O histograma é montado pixel a pixel a partir da intensidade em tons de cinza.
...
void Histogram::draw(SDL_Renderer* renderer, int x, int y, int width, int height) const {
    ...
    // O fundo é redesenhado a cada quadro para não deixar resíduos da atualização anterior.
...
void Histogram::drawWithOverlay(SDL_Renderer* renderer,
                                int x,
                                int y,
                                int width,
                                int height,
    ...
    // A normalização usa o maior pico entre os dois histogramas para manter a comparação justa.
    ...
    // A imagem base em cinza fica ao fundo e o estado atual é desenhado por cima.
...
[[nodiscard]] bool Histogram::savePlotImage(const std::string& file_path,
    ...
    // A área útil deixa margem para os eixos e evita que as barras encostem na borda.
    const int padding_bottom = std::max(
        histogram_constants::MIN_PADDING_BOTTOM,
        static_cast<int>(image_height * histogram_constants::PADDING_BOTTOM_RATIO)
    );
    const int padding_top = std::max(
        histogram_constants::MIN_PADDING_TOP,
    ...
    // Cada faixa de intensidade é convertida em uma barra vertical proporcional à sua frequência.
    for (int intensity = 0; intensity < histogram_constants::INTENSITY_LEVELS; ++intensity) {
